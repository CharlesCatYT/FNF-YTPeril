...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (FlxG.random.bool(2))
			noModsTxt.text += '\nBITCH.'; // meanie
		noModsTxt.setFormat(Paths.font("youtube.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
class ModMetadata
{
	...
	public function new(folder:String)
	{
		...
		if (pack != null)
		{
			...
			if (pack.color != null)
				this.color = FlxColor.fromRGB(pack.color[0] != null ? pack.color[0] : 170, pack.color[1] != null ? pack.color[1] : 0,
