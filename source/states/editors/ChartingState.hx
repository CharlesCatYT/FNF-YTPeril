...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'Dadbattle Spotlight',
			"Used in Dad Battle,\nValue 1: 0/1 = ON/OFF,\n2 = Target Dad\n3 = Target BF\n \nOnly works in the Week 1 stage."
		],
		[
			'Hey!',
		...
		['Change Icon', "Value 1: Player 1 BF Icon\nValue 2: Player 2 Opponent Icon"],
		[
			'Change Scroll Speed',
	...
	**/
	...
	override function create()
	{
		...
		lilStage = new FlxSprite(32, 432).loadGraphic(Paths.image("lilStage")); // hi rozebud
		lilStage.scrollFactor.set();
		add(lilStage);

		lilBf = new FlxSprite(32, 432).loadGraphic(Paths.image("lilBf"), true, 300, 256); // hi rozebud
		...
		lilOpp = new FlxSprite(32, 432).loadGraphic(Paths.image("lilOpp"), true, 300, 256); // hi rozebud
		...
		if (!blockInput)
		{
			...
			if (!vortex)
			{
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN)
				{
					...
					var snap:Float = quantization / 4;
					var increase:Float = 1 / snap;
					FlxG.sound.music.time = Conductor.beatToSeconds(FlxG.keys.pressed.UP ? CoolUtil.quantize(beat,
						snap) - increase : CoolUtil.quantize(beat, snap)
						+ increase);
				}
			}
